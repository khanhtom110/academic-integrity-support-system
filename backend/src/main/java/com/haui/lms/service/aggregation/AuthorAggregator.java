...
public class AuthorAggregator {
    ...
    public void aggregateBatch(
        ...
        for (OpenAlexWorkDTO work : works) {
            ...
            for (AuthorshipDTO authorship :
                ...
                accumulator.authorNames.putIfAbsent(
                        authorId,
                        authorName
                );


                accumulator.authorTotals.merge(
                        authorId,
                        1,
                        Integer::sum
                );
    ...
    public AuthorAggregationDTO finish(
            Accumulator accumulator) {


        //1. SORT AUTHOR
        List<String> sortedAuthors =
                accumulator.authorTotals
                        .keySet()
                        .stream()
                        .sorted((a, b) -> {
        ...
        for (Map<Integer, Integer> yearly :
                accumulator.authorYear.values()) {
        ...
        for (String authorId :
                topAuthorIds) {
            Map<Integer, Integer> yearlyData =
                    new HashMap<>();

            Map<Integer, Integer> existingData =
                    accumulator.authorYear
                            .getOrDefault(
                                    authorId,
        ...
        AuthorAggregationDTO result =
                new AuthorAggregationDTO();
