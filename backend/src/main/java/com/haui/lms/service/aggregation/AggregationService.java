...
public class AggregationService {
    ...
    public AggregationService(CountryAggregator countryAggregator, InstitutionAggregator institutionAggregator, AuthorAggregator authorAggregator) {
    ...
    public AggregationAccumulator createAccumulator() {
        return new AggregationAccumulator(
                countryAggregator.createAccumulator(),
                institutionAggregator.createAccumulator(),
                authorAggregator.createAccumulator()
        );
    }
    ...
    public void aggregateBatch(AggregationAccumulator accumulator, List<OpenAlexWorkDTO> works) {
        ...
        for (OpenAlexWorkDTO work : works) {
            ...
            if (type == null || type.isBlank()) {
                ...
                typeKey =
                        type.replaceFirst(
                                        "^https?://openalex\\.org/types/",
                                        ""
                                )
                                .toLowerCase();
            }

            accumulator.contentMix.merge(
                    typeKey,
                    1,
                    Integer::sum
            );

            accumulator.contentMixByYear
                    .computeIfAbsent(
                            year,
                            y -> new HashMap<>()
                    )
        ...
        //INSTITUTION
        institutionAggregator.aggregateBatch(
    ...
    public AggregationDTO finish(AggregationAccumulator accumulator) {
        ...
        //INSTITUTION
        InstitutionAggregationDTO institutionResult =
                institutionAggregator.finish(
                        accumulator.institution,
                        countryResult.getTopCountries()
                );

        calculateInstitutionPercentages(
                institutionResult,
                accumulator.papersPerYear
        );
        ...
        //AUTHOR
        AuthorAggregationDTO authorResult =
                authorAggregator.finish(
                        accumulator.author
                );

        calculateAuthorPercentages(
                authorResult,
                accumulator.papersPerYear
        );
    ...
    private void calculateCountryPercentages(
            CountryAggregationDTO countryResult,
            Map<Integer, Integer> papersPerYear) {

        if (countryResult == null ||
                countryResult.getData() == null) {
        ...
        for (Map.Entry<String, Map<Integer, Double>> entry :
                countryResult.getData().entrySet()) {
        ...
        countryResult.setPercentages(
                percentages
        );
    }

    private void calculateInstitutionPercentages(
            InstitutionAggregationDTO institutionResult,
            Map<Integer, Integer> papersPerYear) {

        if (institutionResult == null ||
                institutionResult.getData() == null) {

            return;
        }

        for (InstitutionCountryDTO countryData :
                institutionResult
                        .getData()
                        .values()) {

            if (countryData == null ||
                    countryData.getData() == null) {
            ...
            Map<Integer, Double>
                    countryPapers =
                    countryData.getPapersPerYear();

            for (Map.Entry<String, Map<Integer, Double>> entry :
                    countryData.getData().entrySet()) {
            ...
            countryData.setPercentages(
                    percentages
            );
        }
    }

    private void calculateAuthorPercentages(
            AuthorAggregationDTO authorResult,
            Map<Integer, Integer> papersPerYear) {

        if (authorResult == null ||
                authorResult.getData() == null) {
        ...
        for (Map.Entry<String, Map<Integer, Integer>> entry :
            ...
            for (Map.Entry<Integer, Integer> yearEntry :
                    entry.getValue().entrySet()) {
    ...
    public static class AggregationAccumulator {
        ...
        private final AuthorAggregator.Accumulator
                author;

        private final CountryAggregator.Accumulator
                country;

        private final InstitutionAggregator.Accumulator
                institution;

        public AggregationAccumulator(
                CountryAggregator.Accumulator country,
                InstitutionAggregator.Accumulator institution,
                AuthorAggregator.Accumulator author) {
