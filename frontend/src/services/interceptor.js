...
/**
 * Server trả 403 với body rỗng (từ Spring Security)
 * thay vì 401 khi access token hết hạn hoặc bị vô hiệu hoá.
 * 403 nghiệp vụ từ API luôn có body ApiResponse.
 */
...
apiClient.interceptors.response.use(
  ...
  async (error) => {
    ...
    /**
     * Không phải lỗi xác thực (401 hoặc 403 body rỗng)
     */
    if (!isAuthRejection(error.response)) {
      return Promise.reject(error);
    }
